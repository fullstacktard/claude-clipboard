...
function askQuestion(question) {
  ...
  return new Promise((resolve) => {
    ...
    const prompt = chalk.red("❯ ") + chalk.white(question) + chalk.gray(" (Y/n): ");
...
export async function install() {
  ...
  showSection("Installation Steps".rocket);
  ...
  if (ahkInstalled) {
    ...
    step4.stop();
    showWarning("AutoHotkey v2 not found");

    const shouldInstall = await askQuestion("Download and install AutoHotkey v2?");
    ...
    if (shouldInstall) {
      ...
      showInfo("Skipping AutoHotkey installation");
      showInfo("You can install manually from: https://www.autohotkey.com/");
    }
  }
  ...
  if (ahkInstalled) {
    const shouldStart = await askQuestion("Start the clipboard monitor now?");
    ...
    if (shouldStart) {
      ...
      showInfo("Skipping clipboard monitor start");
      showInfo(`You can start it later by running: cd "${windowsUserProfile}\\AppData\\Local\\claude-clipboard" && .\\start-smart-paste.ps1`);
    }
  }
  ...
  if (ahkInstalled) {
    const shouldAutoStart = await askQuestion("Configure clipboard monitor to start on Windows login?");
    ...
    if (shouldAutoStart) {
      ...
      showInfo("Skipping auto-start configuration");
    }
  }

  // Installation complete
  showSection("Installation Complete!".sparkles);

  const isFullySetup = ahkInstalled && monitorStarted && autoStartConfigured;
  const needsManualSteps = !ahkInstalled || !monitorStarted;

  showBox(
    `${isFullySetup ? "Ready to Use!" : "Setup Complete"}`,
...
${ahkInstalled ? chalk.green("✓") : chalk.yellow("⚠")} AutoHotkey v2: ${ahkInstalled ? "Installed" : "Manual installation required"}
${monitorStarted ? chalk.green("✓") : chalk.dim("○")} Clipboard monitor: ${monitorStarted ? "Running" : "Not started"}
${autoStartConfigured ? chalk.green("✓") : chalk.dim("○")} Auto-start on login: ${autoStartConfigured ? "Configured" : "Not configured"}

${chalk.bold("How to Use:")}

${chalk.cyan("1.")} Take a screenshot ${chalk.dim("(Win+Shift+S)")}
${chalk.cyan("2.")} Open Claude Code in Windows Terminal
${chalk.cyan("3.")} Press ${chalk.magenta("Ctrl+V")}
${chalk.cyan("4.")} Screenshot path appears automatically!

${chalk.bold("Installed Files:")}
${chalk.dim(`WSL:     ${installDir}/`)}
${chalk.dim(`Windows: ${windowsUserProfile}\\AppData\\Local\\claude-clipboard\\`)}

${needsManualSteps ? `\n${chalk.yellow.bold("Next Steps:")}\n${!ahkInstalled ? `1. Install AutoHotkey v2: ${chalk.white("https://www.autohotkey.com/")}\n` : ""}${!monitorStarted ? `${!ahkInstalled ? "2" : "1"}. Start monitor: ${chalk.white(`cd "${windowsUserProfile}\\\\AppData\\\\Local\\\\claude-clipboard" && .\\\\start-smart-paste.ps1`)}\n` : ""}` : ""}${chalk.dim("For troubleshooting: https://github.com/fullstacktard/claude-clipboard#readme")}`,
